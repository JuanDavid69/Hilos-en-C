...
double partialSums[MAXTHREAD]; // Variable que tiene la suma parcial de c/hilo
...
void* calcular(void *arg) {
	...
	//    variable 'loslimit'
	double loslimit;
	loslimit = base_length * numRectxThread * id;
	...
	for(int i = 0; i < numRectxThread; i++){
		partialSums[id] = partialSums[id] + (function(loslimit + i*base_length) * base_length);
	}
